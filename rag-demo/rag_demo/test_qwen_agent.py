...
[智谱 AI 集成] 两种使用方式：
1. 方式一：直接传入 CustomChatZhipuAI 对象（推荐）
   - CustomChatZhipuAI 同时支持 LangChain 和 Qwen-Agent
   - 所有配置从 config.yaml 读取（内聚设计）
   - 自动从配置文件读取代理和 SSL 设置

2. 方式二：传入配置字典
   - 使用 model_type='zhipuai'
   - Qwen-Agent 会自动创建 CustomChatZhipuAI 实例
"""

from pathlib import Path
from qwen_agent.agents import Assistant
from ai_starter import Config, get_logger, CustomChatZhipuAI
...
class QwenAgentPipeline:
    ...
    def __init__(self):
        ...
        # 方式一：直接传入 CustomChatZhipuAI 对象（推荐）
        # CustomChatZhipuAI 所有配置从 config.yaml 读取
        logger.info("使用 CustomChatZhipuAI 对象（配置从 config.yaml 读取）")
